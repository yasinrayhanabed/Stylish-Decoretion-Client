...
export default function AdminManageDecorators() {
  ...
  const paginatedDecorators = filteredDecorators.slice(
    startIndex,
  ...
  useEffect(() => {
    const fetchDecorators = async () => {
      try {
        ...
        const decoratorUsers = allUsers.filter(
          (user) => user.role === "decorator"
        );
        ...
        if (err.response?.status === 403) {
          ...
          setError(
            "Failed to load decorators. Please check server connection."
          );
        }
        setDecorators([]);
        setLoading(false);
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-blue-100 text-lg">
              Oversee and manage professional decorators
            </p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row flex-wrap justify-between items-start sm:items-center gap-2 text-sm">
          <div className="flex items-center">
            <span className="text-gray-600 font-medium">
              ...
              of <span className="font-bold">{decorators.length}</span>{" "}
              decorators
            </span>
          </div>
          {searchTerm && (
            <span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full font-medium inline-block mt-2 sm:mt-0">
              🔍 "{searchTerm}"
            </span>
          )}
        </div>
          ...
          <>
            <div className="grid gap-4">
              {paginatedDecorators.map((decorator, index) => (
                <div
                  key={decorator._id}
                  className="bg-gradient-to-r from-white to-gray-50 p-4 md:p-6 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 border border-gray-100 hover:border-indigo-200"
                >
...
📊 Status: Active
                          ...
                          className="px-3 py-2 bg-blue-100 hover:bg-blue-200 text-blue-700 rounded-lg transition-colors flex items-center font-medium text-sm"
                        ...
                        <button
                          onClick={() => {
                            if (
                              window.confirm(
                                `Are you sure you want to remove decorator role from ${decorator.name}?`
                              )
                            ) {
                              handleToggleStatus(decorator._id, true);
                            }
                          }}
                          className="px-3 py-2 bg-red-100 hover:bg-red-200 text-red-700 rounded-lg transition-colors flex items-center font-medium text-sm"
                      ...
                      <button
                        ...
                        className="w-full sm:w-auto px-3 py-1 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded text-xs transition-colors"
