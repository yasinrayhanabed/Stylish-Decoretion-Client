...
export default function AdminManageBookings() {
  ...
  const refreshBookings = async () => {
    try {
      const bookingsRes = await API.get("/bookings").catch(() =>
        API.get("/bookings/my")
      );
      ...
      const sanitizedBookings = bookingsData.map((booking, index) => {
        ...
        // Fix: Ensure assignedDecorator is set if backend returns 'decorator'
        if (!sanitized.assignedDecorator) {
          if (booking.decorator) {
            sanitized.assignedDecorator =
              typeof booking.decorator === "object"
                ? booking.decorator._id
                : booking.decorator;
          } else if (booking.decoratorId) {
            sanitized.assignedDecorator = booking.decoratorId;
          }
        }
        return sanitized;
      });
  ...
  const handleAssignDecorator = async () => {
    ...
    try {
      ...
      await API.put(`/bookings/${selectedAssignment.bookingId}`, {
        ...
        decorator: selectedAssignment.decoratorId,
        status: "Assigned",
      });
      ...
      toast.success(
        `Decorator ${selectedAssignment.decoratorName} assigned successfully!`
      );
  ...
  const handlePaymentStatusToggle = async (bookingId, currentStatus) => {
    ...
    try {
      await API.put(`/bookings/${bookingId}`, {
        ...
        paymentStatus: newPaymentStatus ? "completed" : "pending",
      });
      setBookings((prev) =>
        prev.map((b) =>
      ...
      toast.success(
        `Payment status updated to ${newPaymentStatus ? "Paid" : "Pending"}`
      );
    } catch (err) {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-4xl font-bold mb-2">
              <FaClipboardList className="inline mr-2" /> Booking Management
            </h2>
            <p className="text-cyan-100 text-lg">
              All customer orders and decorator assignments
            </p>
          </div>
                      ...
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-500">
                            {booking._id && booking._id.length >= 16
                              ? booking._id.substring(8, 16)
                              : "Generated ID"}
                          </div>
                        </div>
                      </div>
                          ...
                          <div className="bg-green-50 border border-green-200 rounded-lg p-2">
                            <div className="flex items-center mb-1">
                              <div className="w-8 h-8 bg-gradient-to-r from-purple-400 to-pink-500 rounded-full flex items-center justify-center text-white text-xs font-bold mr-2">
                                {(
                                  decorators.find(
                                    (d) => d._id === booking.assignedDecorator
                                  )?.name || "D"
                                ).charAt(0)}
                              </div>
            ...
            <div className="space-y-3 mb-6">
              <div className="bg-blue-50 p-3 rounded-lg border border-blue-200">
                ...
                <div className="text-blue-900 font-bold">
                  {selectedAssignment.serviceName}
                </div>
              </div>
              ...
              <div className="bg-green-50 p-3 rounded-lg border border-green-200">
                ...
                <div className="text-green-900 font-bold">
                  {selectedAssignment.decoratorName}
                </div>
              </div>
            </div>

            <div className="text-sm text-gray-600 mb-6 bg-yellow-50 p-3 rounded-lg border border-yellow-200">
              ⚠️ Are you sure you want to assign this decorator to this booking?
